...
class AlgoritmoGenetico:
    ...
    def maxLen(self, individuos: list, binario: bool):
        maximo: int = 13 if binario else 9
        for i in individuos:
            if binario:
                iLen = len(format(i, "030b"))
            else:
                iLen = len(str(i))
            if (iLen > maximo):
                maximo = iLen
    ...
    def printGeneracionActual(self) -> None:
        l1: int = len(str(self.cant_individuos))
        if l1 < 3: l1 = 3
        l2: int = self.maxLen(self.poblacion, False)
        l3: int = self.maxLen(self.poblacion, True)
        l4: int = 11
        l5 = l1 + l2 + l3 + l4 + 15
        sep = " | "
        print("-" * l5)
        print(sep +
              "Ind".center(l1, " ") + sep +
              "Cromosoma".center(l2, " ") + sep +
              "Cromosoma(0b)".center(l3, " ") + sep +
              "Fitness".center(l4) + sep)
        print("-" * l5)
        i: int = 0
        for individuo in self.poblacion:
            i += 1
            print(sep +
                  str(i).rjust(l1, " ") + sep +
                  str(individuo).rjust(l2, " ") + sep +
                  format(individuo, "030b").ljust(l3, " ") + sep +
                  format(self.list_fitness[i-1], "1.9f").ljust(l4, " ") + sep)
        print("-" * l5)
